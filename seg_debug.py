...
from utils.vis import color_encode
...
def pred_merge(img, pred, mask_alpha=0.3):
    """Merge mask to pic."""
    img = np.array(img).astype(np.float32)
    idx = np.nonzero(pred)

    mask_color = color_encode(pred, colors)
...
def main():
    ...
    for i in range(1):
        ...
        # type(pred): numpy.ndarray; type(img): PIL.Image.Image
        # print('pred:', type(pred), pred.shape, 'img:', type(img), img.size)
        # pred = pred.reshape((img[0], img[1], 3))

        img = pred_merge(img, pred)
        img_name = 'P000014_debug.png'
        Image.fromarray(img).save(img_name, quality=95)
